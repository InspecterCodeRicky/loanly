...
module.exports = (plugin) => {
    ...
    plugin.controllers.auth.register = async (ctx) => {
        ...
        var { society } = ctx.request.body

        if (!society) {
            return ctx.badRequest("Society codeSociety must be defined.")
        }

        var societyExists = await strapi.db.query('api::society.society').findOne({
            where: {
                $and: [
                    {
                        codeSociety: society
        ...
        ctx.request.body.society = societyExists
